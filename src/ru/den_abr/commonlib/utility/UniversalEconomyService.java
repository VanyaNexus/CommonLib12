...
import ru.den_abr.commonlib.events.EconomyReadyEvent;
...
public class UniversalEconomyService implements Listener
{
    ...
    public void init() {
        this.log = CommonLib.INSTANCE.getLogger();
        ...
        try {
            ...
            (this.providers = (Map<Class<?>, List<RegisteredServiceProvider<?>>>)providersField.get(this.servicesManager)).put(Economy.class, new StaticList((RegisteredServiceProvider<?>)new RegisteredServiceProvider(Economy.class, this.wrapper, ServicePriority.Highest, CommonLib.INSTANCE)));
        }
        catch (Exception e) {
        ...
        pman.registerEvents(this, CommonLib.INSTANCE);
        pman.callEvent(new EconomyReadyEvent(this));
    }
    ...
    public void onDisable(final PluginDisableEvent e) {
        if (e.getPlugin().getName().equals(CommonLib.INSTANCE.getName())) {
    ...
    public void registerService(final SimpleEconomyService service) {
        ...
        CommonLib.INSTANCE.getLogger().info("Service " + service.getName() + " has been registered.");
        this.ownRegistered = true;
        this.wrapper.setDelegate(service);
    }
    
    public void updateDelegate(final Economy economy) {
        CommonLib.INSTANCE.getLogger().info("Replacing " + this.wrapper.getDelegate().getName() + " service with " + economy.getName());
        this.wrapper.setDelegate(economy);
    }
    
    public static void start() {
        Preconditions.checkState(!CommonLib.isEconomyReady(), "Economy already started.");
        if (CommonLib.CONF.isReplaceVaultEconomyService()) {
            final UniversalEconomyService service = new UniversalEconomyService();
            CommonLib.setEconomyService(service);
            service.init();
            CommonLib.INSTANCE.getLogger().info("Vault enabled. Activated universal economy service.");
        }
    }
    ...
    private class EconomyWrapper implements SimpleEconomyService
    {
        ...
        public void setDelegate(final Economy delegate) {
            this.delegate = (Economy)Preconditions.checkNotNull((Object)delegate);
