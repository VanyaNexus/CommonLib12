...
public class Message {
    ...
    public JsonElement getCompleteValue() {
        ...
        for (Map.Entry<String, Object> tag : this.tags.entrySet()) {
            ...
            try {
                ...
                CommonLib.INSTANCE.getLogger().log(Level.SEVERE, "Got exception processing tag " + full + " with value " + value + " for message " + this.key, e2);
            }
        }
    ...
    public static JsonPrimitive setPrimitive(JsonPrimitive p, Object val) {
        Accessors.getFieldAccessor(JsonPrimitive.class, "value", true).set(p, val);
        return p;
    }
            ...
            case STRING: {
                if (!(cs instanceof Player) || pos == MessagePosition.CHAT) {
                    if(getAsString(el).contains("[jmm|")) {
                        cs.sendMessage(JsonMessageConverter.DEFAULT.convert(getAsString()));
                    } else {
